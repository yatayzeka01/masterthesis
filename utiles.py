...
def plotter(combinedDatatoPlot, width, height):
    ...
    for x in i:
        ...
        for stock in stocks.Stock.unique():
            ...
            plt.annotate(stock, (mdates.date2num(stockDF["date_time"][plotindex]), stockDF[stock][plotindex]), xytext=(15, 15), textcoords='offset points', arrowprops=dict(arrowstyle='-|>', color=c), color=c)
